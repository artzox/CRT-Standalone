...
| `ENABLE_SCANLINE_SS` | 0 | Rotated-grid 4× scanline supersampling — diagonal aliasing mitigation, with optional structure-tensor edge gating |
| `ENABLE_BEAM_H` | 0 | Horizontal beam reconstruction — analytic horizontal spot profile, complement to the vertical scanline beam |
...
| `HBEAM_TAPS` | 2 | Horizontal beam reconstruction tap count per side (requires `ENABLE_BEAM_H=1`). 2 = 5-tap (default, plenty for typical spot sizes), 3 = 7-tap for very wide horizontal beams |
...
- **Horizontal Beam Width / Horizontal Beam Amount** (requires `ENABLE_BEAM_H=1`) — analytic horizontal beam reconstruction, the complement to the vertical scanline beam. Unlike Beam Horizontal Bloom (a luma-gated cosmetic smear on bright pixels only), this reconstructs the true horizontal electron spot profile for *all* content using the same generalized-Gaussian integral and Beam Shape exponent as the vertical scanline. Thin bright vertical lines bloom correctly and hard horizontal transitions gain a natural spot footprint instead of a bilinear edge. **Width** = spot size as a fraction of a source pixel (0.3–0.4 tight/focused, 0.5–0.7 typical consumer CRT, 0.8+ defocused). **Amount** = blend against the sharp source. Tap count set by `HBEAM_TAPS` preprocessor (2 = 5-tap default). Warp- and preblur-aware
...
- **Scanline AA / Scanline AA Edge Gating** (requires `ENABLE_SCANLINE_SS=1`) — rotated-grid 4× supersampling of the scanline beam envelope, targeting diagonal stairstepping. **Scanline AA** sets the supersampling amount (1.0 = full). **Edge Gating** restricts the supersampling to coherent geometric edges using a structure-tensor measure of local gradient alignment: real edges (gradients all aligned) get the full effect while fine texture (gradients in many directions) is left single-sampled. This lets you run Scanline AA at full strength to kill diagonal stairs without smearing texture detail. 1.0 = full gating (recommended), 0.0 = always-on (may blur texture at high strength). Note: vertical Pre-Blur remains the most effective single mitigation for scanline aliasing; this is a complementary geometric approach
